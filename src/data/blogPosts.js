export const blogPosts = [
    {
        id: 1,
        slug: 'art-of-layering-perfume',
        title: 'The Art of Layering: How to Create Your Signature Scent',
        excerpt: 'Discover the secrets to combining fragrances to create a scent that is uniquely yours. Learn which notes complement each other and how to balance intensity.',
        date: 'October 15, 2025',
        author: 'Elyra Scent Team',
        image: '/images/blog-layering.jpg',
        content: `
            <p>Fragrance layering, or "cocktailing," is the art of mixing two or more scents to create a bespoke fragrance that expresses your unique personality. It’s not just about spraying two perfumes on top of each other; it’s about understanding how notes interact.</p>
            
            <h3>Understanding the Basics</h3>
            <p>Start with a heavy scent as your base—think woods, ambers, or musks. These notes linger longest on the skin. Then, apply a lighter, more volatile scent on top, such as citrus, florals, or fruits. This ensures the lighter notes aren't overpowered.</p>

            <h3>Combinations to Try</h3>
            <ul>
                <li><strong>Floral + Woody:</strong> A classic combination. The depth of wood (like sandalwood or oud) grounds the airy sweetness of florals (like rose or jasmine).</li>
                <li><strong>Citrus + Amber:</strong> Bright citrus notes cut through the richness of amber, creating a dynamic and energetic scent profile.</li>
                <li><strong>Vanilla + Spicy:</strong> For a warm, comforting vibe, mix sweet vanilla with spicy notes like cinnamon or pink pepper.</li>
            </ul>

            <h3>Tips for Success</h3>
            <p>Don't be afraid to experiment. Test combinations on a blotter first before applying to your skin. And remember, less is often more—start with one spray of each and adjust as needed.</p>
        `,
        tags: ['Tips & Tricks', 'Layering', 'Scent Guide']
    },
    {
        id: 2,
        slug: 'history-of-oud',
        title: 'Liquid Gold: A Brief History of Oud in Perfumery',
        excerpt: 'Trace the origins of one of the world’s most precious ingredients. From ancient rituals to modern luxury, explore why oud continues to captivate.',
        date: 'November 2, 2025',
        author: 'Sarah Jenkins',
        image: '/images/blog-oud.jpg',
        content: `
            <p>Oud, often referred to as "liquid gold," is one of the most expensive and sought-after ingredients in the world of perfumery. Derived from the resinous heartwood of the agar tree, its scent is complex, woody, animalic, and deeply spiritual.</p>

            <h3>Ancient Origins</h3>
            <p>The use of oud dates back thousands of years to ancient civilizations in the Middle East and Asia. It was used in religious ceremonies, meditation, and traditional medicine. Its smoke was believed to carry prayers to the heavens.</p>

            <h3>Modern Luxury</h3>
            <p>In recent decades, oud has taken the Western perfume world by storm. Niche and designer houses alike have embraced its rich, complex profile. It adds a sense of mystery, luxury, and longevity to fragrances that few other ingredients can match.</p>

            <p>At House of Elyra, our <em>Mystic Oud</em> and <em>Shadow Rift</em> pay homage to this timeless ingredient, blending it with modern notes for a contemporary yet respectful interpretation.</p>
        `,
        tags: ['Ingredients', 'History', 'Oud']
    },
    {
        id: 3,
        slug: 'scent-and-memory',
        title: 'Scent and Memory: Why Fragrance is So Nostalgic',
        excerpt: 'Ever smell a perfume and instantly travel back in time? Science explains the powerful link between our sense of smell and our memories.',
        date: 'November 20, 2025',
        author: 'Dr. Elena Rossi',
        image: '/images/blog-memory.jpg',
        content: `
            <p>Of all our senses, smell is the most intimately linked to memory and emotion. This phenomenon, often called the "Proustian effect," occurs because the olfactory bulb—the part of the brain that processes smells—has direct connections to the amygdala and hippocampus, which handle emotion and memory.</p>

            <h3>The Emotional Connection</h3>
            <p>Unlike sight or sound, which are processed by other parts of the brain first, scent goes straight to the emotional center. This is why a whiff of vanilla might remind you of your grandmother's kitchen, or the scent of rain (petrichor) brings back memories of childhood summers.</p>

            <h3>Choosing a Scent for Memories</h3>
            <p>Many people choose a specific fragrance for their wedding day or a special trip, consciously creating a "scent memory" they can revisit later. By wearing a unique perfume during these times, you anchor those moments to that specific aroma.</p>
        `,
        tags: ['Science', 'Psychology', 'Wellness']
    },
    {
        id: 4,
        slug: 'niche-vs-designer-perfume',
        title: 'Niche vs. Designer Perfumes: What’s the Real Difference?',
        excerpt: 'Unpacking the debate between mass-market appeal and artisanal craftsmanship. Why niche-style fragrances are taking over the luxury market.',
        date: 'December 1, 2025',
        author: 'Elyra Scent Team',
        image: '/images/blog-niche.jpg',
        content: `
            <p>Walk into any department store, and you'll see rows of famous designer bottles. But dig a little deeper into the fragrance world, and you'll find "niche" perfumery. So, what sets them apart?</p>

            <h3>Mass Appeal vs. Artistic Expression</h3>
            <p>Designer fragrances are often created to appeal to the widest possible audience. They are safe, pleasant, and follow current trends. Niche fragrances, on the other hand, are often driven by a specific artistic vision. They might use bolder notes, higher concentrations of natural ingredients, or explore unconventional scent profiles.</p>

            <h3>The Value Proposition</h3>
            <p>While designer scents pay heavily for celebrity marketing and packaging, niche brands (and niche-inspired houses like House of Elyra) often invest more in the "juice" itself. This means you often get better longevity, projection, and ingredient quality for your money.</p>

            <h3>Finding Your Unique Scent</h3>
            <p>If you don't want to smell like everyone else, niche-style perfumes are the way to go. They offer a chance to wear something that feels personal, rare, and distinctly "you."</p>
        `,
        tags: ['Education', 'Niche Perfume', 'Luxury']
    },
    {
        id: 5,
        slug: 'make-perfume-last-longer',
        title: 'Longevity Hacks: How to Make Your Perfume Last All Day',
        excerpt: 'Tired of your scent fading by lunchtime? Here are expert-approved tips to boost the staying power of your favorite fragrances.',
        date: 'December 3, 2025',
        author: 'Isabella Chen',
        image: '/images/blog-longevity.jpg',
        content: `
            <p>We've all been there: you spray your favorite perfume in the morning, and by noon, it's vanished. While the concentration of the perfume (EDP vs. EDT) matters, how you apply it plays a huge role too.</p>

            <h3>Hydrate, Hydrate, Hydrate</h3>
            <p>Fragrance molecules cling better to hydrated skin. Apply an unscented lotion or body oil immediately after showering, then spray your perfume. This creates a "primer" for the scent to hold onto.</p>

            <h3>Target Pulse Points</h3>
            <p>Apply fragrance to warm areas of your body: wrists, neck, inside elbows, and even behind the knees. The heat helps diffuse the scent throughout the day. And whatever you do—<strong>don't rub your wrists together!</strong> This friction breaks down the delicate top notes.</p>

            <h3>Storage Matters</h3>
            <p>Heat, light, and humidity are the enemies of perfume. Storing your bottles in the bathroom is a common mistake. Keep them in a cool, dark place (like a drawer or closet) to preserve their potency and character.</p>
        `,
        tags: ['Tips & Tricks', 'Longevity', 'How-To']
    },
    {
        id: 6,
        slug: 'seasonal-scent-guide',
        title: 'Seasonal Shifts: Transitioning Your Scent Wardrobe',
        excerpt: 'Just as you change your wardrobe with the seasons, your fragrance should evolve too. A guide to picking the perfect notes for every climate.',
        date: 'December 5, 2025',
        author: 'Elyra Scent Team',
        image: '/images/blog-seasons.jpg',
        content: `
            <p>Temperature and humidity drastically affect how a perfume performs. High heat makes scents project more strongly, while cold weather "traps" fragrance close to the skin.</p>

            <h3>Spring & Summer: Light and Airy</h3>
            <p>In warmer months, opt for fresh, citrusy, or aquatic notes. Heavy orientals can become cloying in the heat. Look for notes like bergamot, sea salt, neroli, and light florals. <em>Aegean Star</em> and <em>Oceanic Halo</em> are perfect examples of summer staples.</p>

            <h3>Fall & Winter: Warm and Cozy</h3>
            <p>As the temperature drops, reach for richer, deeper scents. Spices, woods, ambers, and gourmand notes (like vanilla or chocolate) shine in the cold. They provide a sense of warmth and comfort. <em>Ember Noir</em> and <em>Mystic Oud</em> are ideal companions for a crisp winter evening.</p>
        `,
        tags: ['Guides', 'Seasonal', 'Lifestyle']
    }
];
