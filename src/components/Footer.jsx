import React from 'react';
import { Link } from 'react-router-dom';

const Footer = () => {
    return (
        <footer className="bg-elyra-cream-dark text-elyra-earth pt-32 pb-12 border-t border-elyra-taupe/20 relative overflow-hidden">
            {/* Decorative Background */}
            <div className="absolute top-0 right-0 w-96 h-96 bg-elyra-soft-gold/10 blur-3xl rounded-full pointer-events-none" />

            <div className="container mx-auto px-8 relative z-10">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-16 mb-24">

                    {/* Brand Column */}
                    <div className="space-y-6">
                        <h3 className="text-3xl font-cinzel text-elyra-soft-brown tracking-[0.1em]">ELYRA</h3>
                        <p className="text-elyra-earth/70 text-sm font-light leading-relaxed">
                            Crafting timeless scents for the modern soul. Experience the essence of eternity.
                        </p>
                        <p className="text-elyra-earth/60 text-xs font-light italic pt-2">
                            * Elyra is our proprietary wellness app. Access is granted exclusively upon purchase.
                        </p>
                        {/* Social Media Icons */}
                        <div className="flex space-x-5 pt-4">
                            <a href="https://www.facebook.com/profile.php?id=61584381554924" target="_blank" rel="noopener noreferrer" className="text-elyra-earth/50 hover:text-elyra-soft-brown transition-colors duration-300">
                                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" /></svg>
                            </a>
                            <a href="https://www.instagram.com/houseofelyra_/" target="_blank" rel="noopener noreferrer" className="text-elyra-earth/50 hover:text-elyra-soft-brown transition-colors duration-300">
                                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zm0-2.163c-3.259 0-3.667.014-4.947.072-4.358.2-6.78 2.618-6.98 6.98-.059 1.281-.073 1.689-.073 4.948 0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98-1.281-.059-1.69-.073-4.949-.073zm0 5.838c-3.403 0-6.162 2.759-6.162 6.162s2.759 6.163 6.162 6.163 6.162-2.759 6.162-6.163c0-3.403-2.759-6.162-6.162-6.162zm0 10.162c-2.209 0-4-1.79-4-4 0-2.209 1.791-4 4-4s4 1.791 4 4c0 2.21-1.791 4-4 4zm6.406-11.845c-.796 0-1.441.645-1.441 1.44s.645 1.44 1.441 1.44c.795 0 1.439-.645 1.439-1.44s-.644-1.44-1.439-1.44z" /></svg>
                            </a>
                            <a href="#" className="text-elyra-earth/50 hover:text-elyra-soft-brown transition-colors duration-300">
                                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24"><path d="M23.953 4.57a10 10 0 01-2.825.775 4.958 4.958 0 002.163-2.723c-.951.555-2.005.959-3.127 1.184a4.92 4.92 0 00-8.384 4.482C7.69 8.095 4.067 6.13 1.64 3.162a4.822 4.822 0 00-.666 2.475c0 1.71.87 3.213 2.188 4.096a4.904 4.904 0 01-2.228-.616v.06a4.923 4.923 0 003.946 4.827 4.996 4.996 0 01-2.212.085 4.936 4.936 0 004.604 3.417 9.867 9.867 0 01-6.102 2.105c-.39 0-.779-.023-1.17-.067a13.995 13.995 0 007.557 2.209c9.053 0 13.998-7.496 13.998-13.985 0-.21 0-.42-.015-.63A9.935 9.935 0 0024 4.59z" /></svg>
                            </a>
                        </div>
                    </div>

                    {/* Shop Column */}
                    <div>
                        <h4 className="text-xs font-semibold uppercase tracking-[0.2em] mb-8 text-elyra-soft-brown/80">Shop</h4>
                        <ul className="space-y-4 text-sm text-elyra-earth/70 font-light">
                            <li><Link to="/discover" className="hover:text-elyra-soft-gold transition-colors duration-300">All Perfumes</Link></li>
                            <li><a href="#" className="hover:text-elyra-soft-gold transition-colors duration-300">New Arrivals</a></li>
                            <li><a href="#" className="hover:text-elyra-soft-gold transition-colors duration-300">Gift Sets</a></li>
                            <li><a href="#" className="hover:text-elyra-soft-gold transition-colors duration-300">Discovery Kit</a></li>
                        </ul>
                    </div>

                    {/* Support Column */}
                    <div>
                        <h4 className="text-xs font-semibold uppercase tracking-[0.2em] mb-8 text-elyra-soft-brown/80">Support</h4>
                        <ul className="space-y-4 text-sm text-elyra-earth/70 font-light">
                            <li><a href="mailto:houseofelyra@gmail.com" className="hover:text-elyra-soft-gold transition-colors duration-300">Contact Us</a></li>
                            <li><a href="#" className="hover:text-elyra-soft-gold transition-colors duration-300">Shipping & Returns</a></li>
                            <li><a href="#" className="hover:text-elyra-soft-gold transition-colors duration-300">FAQ</a></li>
                            <li><a href="#" className="hover:text-elyra-soft-gold transition-colors duration-300">Store Locator</a></li>
                        </ul>
                    </div>

                    {/* Newsletter Column */}
                    <div>
                        <h4 className="text-xs font-semibold uppercase tracking-[0.2em] mb-8 text-elyra-soft-brown/80">Newsletter</h4>
                        <p className="text-elyra-earth/70 text-sm font-light mb-6 leading-relaxed">
                            Subscribe to receive updates, access to exclusive deals, and more.
                        </p>
                        <form className="flex flex-col space-y-4">
                            <input
                                type="email"
                                placeholder="Enter your email"
                                className="bg-elyra-cream/50 border border-elyra-taupe/20 px-5 py-4 text-sm text-elyra-earth placeholder:text-elyra-earth/40 focus:outline-none focus:border-elyra-soft-gold transition-colors duration-300"
                            />
                            <button className="bg-elyra-soft-brown text-elyra-cream px-6 py-4 text-xs uppercase tracking-[0.2em] font-semibold hover:bg-elyra-earth transition-colors duration-300">
                                Subscribe
                            </button>
                        </form>
                    </div>
                </div>

                {/* Bottom Bar */}
                <div className="pt-10 border-t border-elyra-taupe/20 flex flex-col md:flex-row justify-between items-center text-xs text-elyra-earth/60 uppercase tracking-[0.15em]">
                    <p>© 2024 House of Elyra. All rights reserved.</p>
                    <div className="flex space-x-8 mt-6 md:mt-0">
                        <a href="#" className="hover:text-elyra-earth transition-colors duration-300">Privacy Policy</a>
                        <a href="#" className="hover:text-elyra-earth transition-colors duration-300">Terms of Service</a>
                    </div>
                </div>
            </div>
        </footer>
    );
};

export default Footer;
